...
var mask = document.querySelector('.mask'); // 鼠标移入移除展示图片时放大镜图片的显示与影藏
...
smallImgBox.onmousemove = function (e) {
  ...
  var maskTop = e.pageY - offset(smallImgBox).top - mask.clientHeight / 2; // 限制 mask 出移动范围
  ...
  mask.style.top = maskTop + 'px'; // 计算小图移动的百分比

  var scaleX = maskLeft / (smallImgBox.clientWidth - mask.clientWidth);
  var scaleY = maskTop / (smallImgBox.clientHeight - mask.clientHeight); // 大图跟着移动(大图移动反向跟小图移动反向相反)
